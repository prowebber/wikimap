...
function showGraph(jsonResponse) {
	...
	Graph.graphData().nodes.forEach(function (node) {
		...
		console.log('Attempted to add sprite for' + node.name);
		// if (!node_obj) return;
		console.log(node.name + " properties: " + Object.getOwnPropertyNames(node));
		console.log(node.name + " values: " + Object.values(node));
		var obj = void 0;
		var node_label = new SpriteText(node.name);
		node_label.color = 'teal';
		obj = node_label;
		obj = obj.clone();
		// var pos = state.layout.getNodePosition(node[state.nodeId]);
		var pos = node;
