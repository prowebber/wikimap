...
function wikiEvents(e) {
		...
		if(e.type == 'keyup'){
			console.log("Keydown");
			let doneTypingTime = 100;                                                       // Specify the time (in ms) to wait before the user is done typing
			clearTimeout(typing_timer);                                             		// Reset the timer
			typing_timer = setTimeout(showSphinxResults, doneTypingTime);         			// Wait n-milliseconds and then run the specified function
		}
			...
			if(e.type == 'click' || e.type == 'submit' || e.type == 'change'){
...
function wikimap() {
	...
	console.log('offline request reached');
	// offlineRequest();
}
wikimap();