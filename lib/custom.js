...
function showGraph(jsonResponse) {
	...
	const Graph = ForceGraph3D()(elem)
		...
		.onNodeClick(node => {
			...
			Graph.cameraPosition({ x: node.x + 40, y: node.y + 40, z: node.z + 40 }, node);
			colorNode(node);
		})
    ...
    Graph.graphData({nodes: Graph.graphData().nodes,links: Graph.graphData().links});
}
