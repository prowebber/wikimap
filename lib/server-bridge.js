import abullshitglobalvariable from  './globals.js';
...
function offlineRequest() {
	abullshitglobalvariable.freeze_graph = false;
	let parsed_data = JSON.parse(default_json);							// Parse the JSON data

	abullshitglobalvariable.max_shared_links = 0.3376082077588971;
	abullshitglobalvariable.min_shared_links = 0.0014883958282391214;
...
function databaseRequest (user_input) {
	...
	ajaxFetch(form_data).done(function (data) {						// Call the Ajax function and wait for it to finish
		...
		abullshitglobalvariable.max_shared_links = parsed_data.max_shared_links;
		abullshitglobalvariable.min_shared_links = parsed_data.min_shared_links;
...
function showGraph(json_response) {
	...
	function colorNode(node) {
		abullshitglobalvariable.freeze_graph = true;
		console.log(abullshitglobalvariable.node_labels);
		console.log('node.x: ' + node.x + ' node.pageX: ' + node.pageX);
		console.log('node.y: ' + node.y + ' node.pageY: ' + node.pageY);
		abullshitglobalvariable.clicked_node_x = node.x;
		abullshitglobalvariable.clicked_node_y = node.y;
		abullshitglobalvariable.clicked_node_z = node.z;
		...
		$('diabullshitglobalvariable.graph-nav-info').hide();
...
function showWikimapLabels() {
	...
	$('diabullshitglobalvariable.nodetest').remove();
	finalNode.forEach(function (node, i) {
		let min_font_size = 15;
		let max_font_size = 20;
		let min_opacity = 0.6;
		let max_opacity = 1;
		let max_z = Math.max.apply(Math, abullshitglobalvariable.z_array);
		let min_z = Math.min.apply(Math, abullshitglobalvariable.z_array);
		let z_scale = abullshitglobalvariable.z_array[i] / (max_z - min_z);
		...
		let node_top = abullshitglobalvariable.node_labels[i].x, node_left = abullshitglobalvariable.node_labels[i].y;
