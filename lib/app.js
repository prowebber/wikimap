...
export default Kapsule({
	...
	init: function(domNode, state) {
		...
		state.forceGraph.onFinishLoading(() => {
			...
			// state.forceGraph.d3Force.forceLink(state.forceGraph.graphData().links).strength = function(link){
			// 	return 3 * link.val;
			// 	};
		});
		...
		// 	v.frame_id = requestAnimationFrame(animate);
		// }
		// domNode.addEventListener("mousedown",animate);
		// Setup tooltip
		...
		function animate() { // IIFE
			...
			if (!v.freeze_graph){
				v.frame_id = state.animationFrameRequestId = requestAnimationFrame(animate);
			}
		};
		v.frame_id = state.animationFrameRequestId = requestAnimationFrame(animate);
	},
