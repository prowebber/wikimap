...
function wikiEvents(e) {
	...
	if(e.type == 'mousemove') {
		let cur = (typeof elem === 'undefined') ? l.id.getDom('3d-graph') : elem;
		let canvas = null;

		if(cur){
			canvas = l.dom.findBySelector(cur, 'canvas');
			canvas.style.position = 'relative';
		}

		console.log('canvas: ', canvas);

		var mouseCoords = l.getMouseCoordinates(cur, e);
		// v.cursorPos = {x: mouseCoords.doc.left, y: mouseCoords.doc.top};

		var mCoords = {x: mouseCoords.doc.left, y: mouseCoords.doc.top};

		console.log("mouseCoords: ", mCoords);

		// update the mouse pos
		var offset = getOffset(cur),
			relPos = {
				x: e.pageX - offset.left,
				y: e.pageY - offset.top
			};

		var domRect = cur.getBoundingClientRect();

		if(domRect.width){
			v.cursorPos.x = relPos.x / domRect.width * 2 - 1;
			v.cursorPos.y = -(relPos.y / domRect.height) * 2 + 1;
		}

		var canvasCoords = (canvas.offsetTop - canvas.scrollTop + canvas.clientTop);

		console.log('canvas coords: y' + canvasCoords);
		console.log('window.width: ', window.width);
		// console.log('x: ' + v.cursorPos.x + ', y: ' + v.cursorPos.y);
	}
...
// console.log('cursorPos: ', v.cursorPos);
...
function showGraph(jsonResponse) {
	...
	v.animate = function animate() {
		...
		mousePos.x = v.cursorPos.x;
		mousePos.y = v.cursorPos.y;
		...
		// console.log('Graph: ', Graph);
		// var intersects = raycaster.intersectObjects(Graph.children, true).map(function (_ref) {
		// 	var object = _ref.object;
		// 	return object;
		// }).sort(hoverOrderComparator);
