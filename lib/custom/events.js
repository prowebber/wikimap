...
function wikiEvents(e) {
	...
	if (e.type == 'submit') {
		...
		if(data.dom.div){
			...
			if(div.id == '3d-graph'){                       // If the user clicks on the 3d-graph
				...
				if (typeof(v.hoverObj)){
					show_wiki(v.hoverObj.__data);
					colorNode(v.graph,v.hoverObj);
				}

				// v.hoverObj.__data.__threeObj.material.color = '#00ff00';
	...
	if(e.type == 'mousemove') {
		...
		if(v.elem !== null){
			var canvas = l.dom.findBySelector(v.elem, 'canvas');
			canvas.style.position = 'relative';

			var domRect = v.elem.getBoundingClientRect();
...
function showGraph(jsonResponse) {
	v.elem = l.id.getDom('3d-graph');
	const renderer = new WebGLRenderer();
	renderer.setSize(window.innerWidth, window.innerHeight);
	v.elem.appendChild(renderer.domv.element);
	v.elem.appendChild(v.nav_info = document.createv.element('div'));
	v.nav_info.className = 'graph-nav-info';
	v.nav_info.textContent = "Left-click: Rotate, Right-click: Pan, Scroll: Zoom";
	v.elem.appendChild(v.infov.elem = document.createv.element('div'));
	v.infov.elem.className = 'graph-info-msg';
	v.infov.elem.textContent = 'initial message';

	// Setup tooltip
	v.toolTipv.elem = document.createv.element('div');
	v.toolTipv.elem.classList.add('scene-tooltip');
	v.elem.appendChild(v.toolTipv.elem);

	// Graph it
	const Graph = new ThreeForceGraph(v.elem)
		.linkWidth(0.5)
		.cooldownTicks(100)
		.nodeColor('#ff00ff')
	...
	// Add camera controls
	const tbControls = new TrackballControls(camera, renderer.domv.element);
	...
	v.animate = function animate() {
		...
		v.hoverObj = topObject;
		if (intersects.length) {
			// console.log('topObject: ', topObject);
			console.log('topObject name: ', topObject.__data.name);
		}
