...
function wikiEvents(e) {
	...
	if(e.type == 'mousemove'){
		...
		v.cursorPos = l.getMouseCoordinates(cur, e);
		// console.dir(cursorPos);
		console.log("Top: " + v.cursorPos.doc.top + " Left: " + v.cursorPos.doc.left);
	}
...
function showGraph(jsonResponse) {
	...
	elem.addEventListener("mousemove", function (ev) {
		...
		// console.log(Graph);
		mousePos.x = relPos.x / window.innerWidth * 2 - 1;
		mousePos.y = -(relPos.y / window.innerHeight) * 2 + 1;
		// Move tooltip
