...
function wikiEvents(e) {
	...
	if (e.type == 'submit') {
		...
		if (data.dom.h1) {
			if (data.dom.h1.id == 'extend_tiers') {
				...
				l.ajax.post(form_data, function (data) {
                    ...
                    console.log("Node Count: " + parsedData.history.node_counter);
					...
					// console.log('nodes before: ', v.graph.graphData().nodes);
					// console.log('new_data: ', parsedData.new_data);
					v.graph.cooldownTicks(100);
					v.graph.graphData({
						nodes: nodes.concat(parsedData.new_data.nodes),
						links: links.concat(parsedData.new_data.links)
					});
					// console.log('nodes after: ',v.graph.graphData().nodes);
		...
		if (data.dom.ul) {
			...
			if (ul.id == 'sphinx_results_dropdown') {
				data.dom.ul = console.log("Sphinx Dropdown: " + keyword);
	...
	function showSphinxResults() {
		...
		l.ajax.sphinx(form_data, function (data) {
			console.log("Sphinx value: " + v.listener.allowSphinxResults);
			if(v.listener.allowSphinxResults) l.id.updateHtml('sphinx_results', data);
			else hideSphinx();
			v.listener.allowSphinxResults = true;
		});
	}
...
function hideSphinx(){
	...
	v.listener.allowSphinxResults = false;
}
...
function showGraph(jsonResponse) {
	...
	scene.fog = new FogExp2('#000000', 0.002);
	scene.add(v.graph);
	...
	v.animate();
	colorOthers(Graph.graphData().nodes);
}
...
function colorOthers(nodes) {
	nodes.forEach(function (node) {
		...
		// node.fixed = true;			// fix all nodes for next simulation
	});
