...
function showGraph(jsonResponse) {
	...
	// Setup scene
	v.scene = new Scene();
	v.scene.fog = new FogExp2('#000000',0.001);
	v.scene.add(v.graph);
	v.scene.add(new AmbientLight(0xbbbbbb));
	v.scene.add(new DirectionalLight(0xffffff, 0.6));
	// Setup camera
	v.camera = new PerspectiveCamera();
	v.camera.aspect = window.innerWidth/window.innerHeight;
	v.camera.updateProjectionMatrix();
	v.camera.lookAt(Graph.position);
	v.camera.position.z = Math.cbrt(jsonResponse.nodes.length) * 75;
